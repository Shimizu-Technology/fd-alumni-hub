...
class ClassArchiveTest < ActiveSupport::TestCase
  test "syncs known team aliases to represented classes" do
    ...
    four_thirty_five = tournament.teams.create!(class_year_label: "430-5", display_name: "430-5")
    ...
    assert_equal [ 1990, 1993, 1994, 1995 ], shortened_four_thirty_five.reload.class_cohorts.order(:graduation_year).map(&:graduation_year)
    ...
    assert_equal [ 2008, 2015 ], eight_fifteen.reload.class_cohorts.order(:graduation_year).map(&:graduation_year)
