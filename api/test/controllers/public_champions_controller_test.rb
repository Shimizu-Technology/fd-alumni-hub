...
class PublicChampionsControllerTest < ActionDispatch::IntegrationTest
  test "returns champion records and title counts with combined classes separate" do
    ...
    assert_equal 4, body["championRecords"].length

    counts = body["titleCounts"].index_by { |entry| entry["championKey"] }
    assert_equal 1, counts.fetch("02").fetch("titles")
    assert_equal 1, counts.fetch("04").fetch("titles")
    assert_equal 2, counts.fetch("02/04").fetch("titles")
  end
