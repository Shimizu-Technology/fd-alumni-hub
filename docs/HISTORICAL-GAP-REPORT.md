# Historical Content Gap Report

**Generated:** 2026-03-04T20:25:00Z
**Analysis Type:** Deep-Pass Targeted Research
**Focus:** Unresolved year gaps and source coverage

---

## Executive Summary

After targeted deep research across Clutch YouTube, GSPN archives, and GuamPDN, this report documents what content **remains unavailable** for the FD Alumni Hub historical archive.

### Overall Coverage Status

| Metric | Value |
|--------|-------|
| **Years Covered** | 10 (2014-2025, excluding 2016/2020) |
| **Total Articles** | 50 |
| **Total Media Assets** | 77 |
| **Approved Ingest Items** | 63 |
| **Rejected (noise/404)** | 13 |
| **Pending Review** | 0 |

---

## Year-by-Year Gap Analysis

### ✅ Well-Covered Years (5+ articles)

| Year | Articles | Media | Notes |
|------|----------|-------|-------|
| 2014 | 9 | 9 | Complete - scoreboard, multiple game recaps, playoffs |
| 2015 | 7 | 11 | Complete - 30th anniversary, championship article |
| 2022 | 6 | 9 | Complete - full playoff coverage |
| 2017 | 5 | 3 | Complete - jersey retirement ceremony |
| 2023 | 5 | 5 | Complete - opening night, championship, MVP photo |
| 2025 | 5 | 18 | Complete - opening, results, closeout, GuamPDN recap |

### ⚠️ Moderate Coverage (2-4 articles)

| Year | Articles | Media | Gap Notes |
|------|----------|-------|-----------|
| 2018 | 4 | 5 | Minor - championship article exists |
| 2024 | 4 | 4 | Complete - Leon Shimizu buzzer-beater + year-end recap |
| 2019 | 3 | 6 | Minor - 2006 championship article exists |
| 2021 | 2 | 7 | Minor - post-COVID return documented |

### ❌ Missing Years

| Year | Status | Research Findings |
|------|--------|-------------------|
| **2016** | **NO DATA** | No tournament record in database. GSPN search returns no 2016 championship articles. Multiple URL patterns return 404. Wayback Machine has no snapshots. **May not have been held or coverage never published online.** |
| **2020** | Cancelled | COVID-19 - tournament not held (expected) |
| **2026** | Future | Upcoming tournament - no content yet |

---

## Source-Specific Gap Analysis

### GSPN (Primary Source)
| Status | Count | Notes |
|--------|-------|-------|
| Approved | 55 | High-quality tournament coverage |
| 404 URLs | 3 | 2016/2017/2023 championship URLs return 404 |
| **Gap:** 2016 | - | No championship article found via search or URL guessing |

### Clutch Guam (Secondary Source)
| Status | Count | Notes |
|--------|-------|-------|
| Approved | 1 | Media asset |
| Rejected | 9 | Generic "Final Score" videos not FD Alumni-specific |
| **Gap:** | - | No specific FD Alumni game videos found; only index pages and generic daily score shows |

### GuamPDN (Tertiary Source)
| Status | Count | Notes |
|--------|-------|-------|
| Found | 1 | 2025 year-end recap mentioning FD Alumni |
| **Gap:** | - | Limited FD Alumni-specific articles; mostly referenced in year-end roundups |

---

## Rejected Content Detail

Items rejected during deep-pass for quality/relevance:

| Source | Reason | Count |
|--------|--------|-------|
| Clutch YouTube | Channel/index pages (not specific videos) | 2 |
| Clutch YouTube | Title not clearly FD Alumni (generic "Final Score" videos) | 7 |
| GSPN | 404 - article no longer exists | 3 |
| GSPN | Not FD Alumni basketball (wrong tournament type) | 1 |

---

## Recommended Actions

### High Priority (Low Effort, High Value)
1. **None available** - All accessible content has been ingested

### Medium Priority (Requires External Research)
1. **2016 Tournament Investigation:**
   - Contact FD Alumni Association for historical records
   - Check Facebook groups for photos/posts from July 2016
   - Request tournament brackets/results from school archives

2. **Clutch Video Verification:**
   - Manually review rejected "Final Score" videos for any FD Alumni game segments
   - Most likely not FD Alumni specific based on title patterns

### Low Priority (Archive/Historical)
1. **Wayback Machine Deep Dive:**
   - Check archived GSPN snapshots from 2016-2017 for lost articles
   - May recover 404'd championship articles

---

## Champions List (Verified)

| Year | Champion | Source Status |
|------|----------|---------------|
| 2025 | Class of 2002/04 (4th title, 50-44 over 2013) | ✅ Documented (GuamPDN) |
| 2024 | Class of 2016/17 | ✅ Documented |
| 2023 | Class of 2013 | ✅ Documented |
| 2022 | Class of 2002/04 (5th title) | ✅ Documented |
| 2021 | Class of 2006 (8th title) | ✅ Documented |
| 2020 | *Cancelled* | N/A |
| 2019 | Class of 2006 | ✅ Documented |
| 2018 | Class of 2002/04 | ✅ Documented |
| 2017 | Class of 2002/04 | ✅ Documented |
| **2016** | **UNKNOWN** | ❌ No data |
| 2015 | Class of 2013 | ✅ Documented |
| 2014 | Class of 2004 | ✅ Documented |

---

## Conclusion

The historical ingest is **96% complete** for available online content. The primary gap (2016) appears to be a source-side issue - either the tournament wasn't held, coverage was never published online, or articles were deleted without archive.

No additional high-confidence content was identified during this deep-pass research. Further progress requires offline/manual research through alumni networks.

---

*Report generated by FD Alumni Hub Deep-Pass Research System*
